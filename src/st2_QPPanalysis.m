...
dataext='HCPR3gsr_demo'; % extended filename=[data '<em>' ext]; 
...
if runM==1 
    Ng=1; a0=[d2O 'GrpQPP/']; if ~exist(a0,'dir'), mkdir(a0); end % dir2 save GrpQPPs<br /> 
    a=[a0 'interm/']; if ~exist(a,'dir'), mkdir(a); end % save intermediate files 
    indn='Grp';  
elseif runM==2 
    Ng=nsbj; a0=[d2O 'SbjQPP/']; if ~exist(a0,'dir'), mkdir(a0); end % dir2 save SbjQPPs<br /> 
    a=[a0 'interm/']; if ~exist(a,'dir'), mkdir(a); end % save intermediate files 
    indn='Sbj'; 
elseif runM==3
    Ng=nscn; a0=[d2O 'ScanQPP/']; if ~exist(a0,'dir'), mkdir(a0); end % dir2 save ScanQPPs<br /> 
    a=[a0 'interm/']; if ~exist(a,'dir'), mkdir(a); end % save intermediate files 
    indn='Scn'; 
...
for ig=1:Ng % gp id 
    for ip=1:nP % QPP id 
        p2S{ig,ip}=[a dataext indn num2str(ig) '_rbst' num2str(rbstScrn) '_qpp' num2str(ip)]; 
    end 
    p2S0{ig}=[a0 dataext indn num2str(ig) '_rbst' num2str(rbstScrn) '_QPPs']; 
end 
%% Computation 
% delete(gcp('nocreate'));  %Removed Parallel Processing 
% myCluster = parcluster('local'); myCluster.NumWorkers = 12;  %Removed Parallel Processing 
% saveProfile(myCluster);  
% parpool(myCluster.NumWorkers,'IdleTimeout',100000000) %Removed Parallel Processing 
for ig=1:Ng 
...
for ip=1:nP        
    ...
    fprintf([indn '%d-QPP%d %dsec'],ig,ip,round(toc)); 
